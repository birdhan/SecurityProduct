...
<div id="main_container">
 ...
   <script>
    
function delete12(shanid) {
	if (confirm("确定删除用户投票")) {
		
		window.location.href = "${pageContext.request.contextPath }/pickdelete?id="
				+ shanid;
...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit">
    <thead>
    	...
    	<tr>
            ...
            <th scope="col" class="rounded" style="width: 65%;">测试产品id</th>
            <th scope="col" class="rounded">票数</th>
            <th scope="col" class="rounded-q4">操作</th>
        </tr>
    </thead>
        
    <tbody>
    	<c:forEach items="${findvote }" var="abcdef">
    	<tr>
        	
            <td>${abcdef.uname }</td>
            <td style="width: 65%;">${abcdef.snacksid }</td>
            <td>${abcdef.type }</td>
            <td><a onclick="delete12('${abcdef.id}')"><img src="images/user_logout.png" alt="" title="" border="0" /></a></td>
        </tr>
       </c:forEach>
