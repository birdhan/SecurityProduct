...
function delete12(shanid) {
	if (confirm("确定删除此用户")) {
		...
		window.location.href = "${pageContext.request.contextPath }/userdelete?id="
				+ shanid;
...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded" style="width: 65%;">收货地址</th>
            <th scope="col" class="rounded-q4">操作</th>
        </tr>
    </thead>
        
    <tbody>
    	<c:forEach items="${listtwo3 }" var="abcde">
    	<tr>
        	
            <td>${abcde.name }</td>
            <td>${abcde.tel }</td>
            <td style="width: 65%;">${abcde.address }</td>
            <td><a onclick="delete12('${abcde.id}')"><img src="images/user_logout.png" alt="" title="" border="0" /></a></td>
        </tr>
       </c:forEach>
