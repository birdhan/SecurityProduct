...
<div id="main_container">
 ...
   <script>
    
function delete11(shanid) {
	if (confirm("确定删除用户预约")) {
		
		window.location.href = "${pageContext.request.contextPath }/mesdeladmin?id="
				+ shanid;
...
<table id="rounded-corner" summary="2007 Major IT Companies' Profit">
    <thead>
    	<tr>
            ...
            <th scope="col" class="rounded">所属板块</th>
            <th scope="col" class="rounded" style="width: 65%;">内容</th>
            <th scope="col" class="rounded-q4">操作</th>
        </tr>
    </thead>
        
    <tbody>
    	<c:forEach items="${listtwo }" var="abcde">
    	<tr>
        	
            <td>${abcde.uname }</td>
            <td>${abcde.status }</td>
            <td style="width: 65%;">${abcde.message }</td>
            <td><a onclick="delete11('${abcde.id}')"><img src="images/user_logout.png" alt="" title="" border="0" /></a></td>
        </tr>
       </c:forEach>
